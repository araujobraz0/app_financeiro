...
export const getDiasNoMes = (ano, mes) => {
...
export const formatarInputDiaMes = (rawValue) => {
    const digits = String(rawValue || '').replace(/\D/g, '').slice(0, 4);
    if (digits.length <= 2)
        return digits;
    return `${digits.slice(0, 2)}/${digits.slice(2)}`;
};
export const parseDiaMesInput = (rawValue, fallbackMonth, fallbackYear) => {
    const digits = String(rawValue || '').replace(/\D/g, '').slice(0, 4);
...
export const parseDiaMesInputOptional = (rawValue, fallbackMonth, fallbackYear) => {
    const digits = String(rawValue || '').replace(/\D/g, '').slice(0, 4);
    if (!digits)
        return { dia: null, mes: null };
    return parseDiaMesInput(rawValue, fallbackMonth, fallbackYear);
};
export const formatarDiaMesInput = (dia, mes, ano) => {
    if (!dia && !mes)
        return '';
    const mesSeguro = Math.min(12, Math.max(1, Number(mes || 1)));
...
export const formatarDiaMes = (dia, competencia) => {
